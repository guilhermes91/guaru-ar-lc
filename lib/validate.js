...
export function validateContent(data) {
  ...
  for (const [chave, lista] of Object.entries(data)) {
    check(
      !Array.isArray(lista) || lista.length <= MAX_ITENS,
      `A lista "${chave}" passou de ${MAX_ITENS} itens. Remova alguns antes de publicar.`,
    );
  }
  const tamanho = JSON.stringify(data).length;
  ...
  if (list(data.products)) {
    ...
    data.products.forEach((item, i) => {
      ...
      check(
        num(item?.price) && item.price > 0,
        `${at}: informe o preço "a partir de" (um número maior que zero, escrito como 1899, sem "R$" nem pontuação).`,
      );
      check(caminhoDeImagem(item?.image), `${at}: a imagem precisa ser enviada pelo próprio painel.`);
  ...
  if (list(data.reviews)) {
    data.reviews.forEach((item, i) => {
      ...
      check(!ehPlaceholder(item?.area), `Avaliação ${i + 1}: troque "${item?.area}" pelo bairro real do cliente.`);
  ...
  const semRepetir = (itens, pega, rotulo) => {
    ...
    itens.forEach((item) => {
      ...
      if (!valor) return;
      check(!vistos.has(valor), `${rotulo} repetido: "${pega(item)}".`);
      vistos.add(valor);
    });
  };
