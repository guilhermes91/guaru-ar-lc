...
// Teto da plataforma, não escolha nossa: o Workers recusa acima disso com
// "Pbkdf2 failed: iteration counts above 100000 are not supported".
// A recomendação OWASP (600k) não é alcançável aqui.
...
export function json(data, status = 200, headers = {}) {
  return new Response(JSON.stringify(data), {
    ...
    headers: {
      ...
      // public/_headers não se aplica a respostas de Function: precisa ir aqui.
...
export async function getUser(env) {
  ...
  if (!env.ADMIN_EMAIL || !env.ADMIN_PASSWORD) {
    // A mensagem vai para uma rota sem sessão (login/recover): o detalhe fica no log.
    console.error("getUser: ADMIN_EMAIL/ADMIN_PASSWORD não definidos no ambiente.");
    throw new Error("O acesso ao painel ainda não foi configurado. Fale com quem cuida do site.");
  }
...
/**
 * Apaga todas as sessões menos a atual. O KV lista por prefixo, e as sessões
 * do painel são poucas (um único usuário), então a varredura é barata.
 */
export async function encerrarOutrasSessoes(env, tokenAtual) {
  const { keys } = await env.ADMIN_KV.list({ prefix: "session:" });
  await Promise.all(
    keys.filter((k) => k.name !== `session:${tokenAtual}`).map((k) => env.ADMIN_KV.delete(k.name)),
  );
}
...
/**
 * Só o sha do content.json publicado, sem ler o conteúdo.
 *
 * O restaurar é o último recurso de quem está com o content.json ilegível — e
 * era justamente aí que ele falhava, porque pedia o conteúdo inteiro (e o
 * `JSON.parse`) para usar só o sha do commit seguinte.
 */
export async function fetchContentSha(env) {
  const file = await ghFetch(env, `/contents/${CONTENT_PATH}?ref=${branch(env)}`);
  return file.sha;
}

/**
 * Caminhos de arquivo servidos em `public/`, com a barra inicial do site
 * (`/images/uploads/x.webp`). Devolve null quando a árvore vem truncada: nesse
 * caso não dá para afirmar que um arquivo falta, e recusar seria pior.
 */
export async function arquivosPublicos(env) {
  const tree = await ghFetch(env, `/git/trees/${branch(env)}:public?recursive=1`);
  if (tree.truncated) return null;
  return new Set((tree.tree || []).filter((no) => no.type === "blob").map((no) => `/${no.path}`));
}
 ...
 */
export async function deployConcluido(env, desde, commit) {
  try {
    const run = await latestWorkflowRun(env, commit);
    if (run?.status === "completed" && run.conclusion === "success") {
      if (!desde) return true;
      return new Date(run.createdAt).getTime() >= new Date(desde).getTime() - 60_000;
    }
    // Cancelado: outro commit tomou o lugar (o workflow usa cancel-in-progress,
    // e um upload de imagem basta). O run daquele commit nunca mais muda de
    // estado, então sem este ramo o rascunho ficaria preso para sempre.
    if (run?.conclusion === "cancelled") {
      const recente = await latestWorkflowRun(env);
      return recente?.status === "completed" && recente.conclusion === "success";
    }
    return false;
  } catch {
    return false;
...
export async function latestWorkflowRun(env, headSha) {
  // Com headSha, busca o run DAQUELE commit. Sem isso, um upload logo depois da
  // publicação cancela o run dela (concurrency: cancel-in-progress) e ele some
  // da consulta do run mais recente — o painel ficava 6 minutos "publicando".
  const filtro = headSha ? `&head_sha=${headSha}` : "";
  const data = await ghFetch(env, `/actions/runs?branch=${branch(env)}${filtro}&per_page=1`);
